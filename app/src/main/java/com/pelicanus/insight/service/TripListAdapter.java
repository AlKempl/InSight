...
public class TripListAdapter extends BaseAdapter {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(getItem(position).avatar == null) {
            throw new NullPointerException("Автарки нет, Нео. Аватарки нет. Есть только матрица");
        }
        getItem(position).avatar.setImageView((ImageView) convertView.findViewById(R.id.exc_image));
        getItem(position).avatar.Download(true);
